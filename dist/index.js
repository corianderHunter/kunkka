...
import { Keyboard, LayoutAnimation, TouchableWithoutFeedback, View, StyleSheet, } from 'react-native';
...
var Drawer = function (_a) {
    var _b;
    var _c = _a.show, show = _c === void 0 ? false : _c, children = _a.children, from = _a.from, width = _a.width, height = _a.height, _d = _a.onPressMask, onPressMask = _d === void 0 ? function () {
        return;
    } : _d, _e = _a.scroll, scroll = _e === void 0 ? true : _e, onHide = _a.onHide;
    var _f = useState(null), size = _f[0], setSize = _f[1];
    var _g = useState([0, 0]), gestureDisplacement = _g[0], setGestureDisplacement = _g[1];
    ...
    }, [from, height, width]), width1 = _h[0], height1 = _h[1];
    var distance = useMemo(function () {
    ...
    var _j = useState(false), keyboardShown = _j[0], setKeyboardShown = _j[1];
    var _k = useState(0), keyboradHeight = _k[0], setKeyboardHeight = _k[1];
    useEffect(function () {
    ...
    var _l = useState('none'), displayDelay = _l[0], setDisplayDelay = _l[1];
    useEffect(function () {
        LayoutAnimation.easeInEaseOut();
        if (show) {
    ...
    }, [gestureDisplacement, from]), translateY = _m.translateY, translateX = _m.translateX;
    var _o = useState(false), delayRender = _o[0], setDelayRender = _o[1];
    useEffect(function () {
        setTimeout(function () { return setDelayRender(true); }, 500);
    }, []);
    var onGestureStop = useMemo(function () { return function (that) {
        var _a = that.displacement, displacementX = _a[0], displacementY = _a[1];
        if (from === Direction.RIGHT || from === Direction.LEFT) {
    ...
    var onGestureMove = useMemo(function () { return function (that) {
        setGestureDisplacement(that.displacement);
    }; }, []);
    var onGestureSwiper = useMemo(function () { return function (that, directions) {
    ...
    }), gesture = _p.gesture, panReponder = _p.panReponder;
    var updateSize = function (e) {
        var _a = e.nativeEvent.layout, width1 = _a.width, height1 = _a.height;
        setSize({ width: width1, height: height1 });
    };
    return delayRender ? (<View style={{
      ...
      <View onLayout={updateSize} style={_b = {
            display: displayDelay,
            width: width1,
            height: height1,
            backgroundColor: '#fff',
        ...
        _b[from] = distance,
        _b}>
        {scroll ? (<ScrollView {...panReponder.panHandlers}>
            <View style={{ flex: 1, borderWidth: 1 }}>{children}</View>
          </ScrollView>) : (<View {...panReponder.panHandlers} style={{ flex: 1, borderWidth: 1 }}>
            {children}
          </View>)}
      </View>
    </View>) : null;
...
export var SideSwiper = function (_a) {
    var size = _a.size, onSwiper = _a.onSwiper, config = _a.config, boundary = _a.boundary;
    if (!size)
        return null;
    var sideSwiperReponder = useMemo(function () {
    ...
    }, [size]).sideSwiperReponder;
    return (<View {...sideSwiperReponder.panHandlers} style={__assign({}, StyleSheet.absoluteFillObject)}></View>);
};
